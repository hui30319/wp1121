...
import { db } from "@/db";
import { titlesTable } from "@/db/schema";
...
type PostTitleRequest = z.infer<typeof postTitleRequestSchema>;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    postTitleRequestSchema.parse(data);
  } catch (error) {
  ...
  const { username, title, titleFromTime, titleToTime, replyToTitleId } = data as PostTitleRequest;
